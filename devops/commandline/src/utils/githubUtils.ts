...
export async function findPullRequestAssociatedWithCommit({
  sha,
}: {
  sha: string;
}) {
  const { repository } = readGitHubCredentials();
  const { data: pullRequests } = await getApiInstance().get<
    GitHubPullRequest[]
...
export function parseGitHubPullRequestInfoFromCommitMessage(
...
export function parseGitHubPullRequestInfoFromCommitMessageSafely(
  commitMessage: string
) {
  try {

...
import { getChangedWorkspaces, getWorkspacePackageJson } from '../utils/yarnUtils';
...
export class TestCommand extends Command {
  ...
  async execute() {
    const concurrencyStaticAnalysis = pLimit(COMMAND_STATIC_ANALYSIS_CONCURRENCY_LIMIT);
    ...
    const tests1 = workspaceLocations.map(workspaceLocation =>
      concurrencyStaticAnalysis(() => this.testWorkspace(workspaceLocation, 'static_analysis'))
    );
  ...
  private async testWorkspace(workspaceLocation: string, scriptType: 'static_analysis' | 'test') {
    const packageJson = await getWorkspacePackageJson(workspaceLocation);
    const packageScripts = Object.keys(packageJson.scripts ?? {});

    const targetScripts = packageScripts.filter(script => TEST_SCRIPTS[scriptType].includes(script));
    ...
    for (const script of targetScripts) {
      await this.runWorkspaceScript(packageJson.name, workspaceLocation, script);
    }
  }

  private async runWorkspaceScript(workspaceName: string, workspaceLocation: string, script: string) {
    const logPrefix = `[${workspaceName}]`;
    ...
    try {
      ...
      await this.reportErrorToGitHubPullRequestComment(workspaceName, workspaceLocation, error as ExecaError);
  ...
  private async reportErrorToGitHubPullRequestComment(
    ...
    const pullRequest = await findPullRequestAssociatedWithCommit({ sha: commitId });
    ...
    if (pullRequest != null) {
      const body = [
        ...
        '에러 트레이스 (stderr):',
        '```',
        stripColor(error.stderr || error.shortMessage || '(stderr가 비어 있습니다)'),
        '```',
      ].join('\n');
